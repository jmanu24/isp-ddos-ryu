...
traffic sourced from each subscriber's own PPP-assigned IP.
...
hping3 cannot transmit anything at all over a ppp-subN interface (its
raw-socket injection assumes Ethernet L2 framing, which a PPP
netdevice doesn't have; see bng_flood.py's own module docstring for
the full diagnosis). ICMP still shells out to the system `ping`
binary, which already worked correctly throughout this project's PPP
debugging (it's genuine kernel-stack traffic, not a raw L2 injection).
...
Usage (on suscriptor, as root -- pppd/ping/raw sockets need it):
...
# out its own ppp-subN interface (see bng_flood.py's module docstring;
# start_attack() below relies on this for TCP_SYN/UDP).
...
class Subscriber:
    ...
    macvlan) and (when attacking) its attack-traffic process (ping, or
    ...
    def _add_source_route(self) -> None:
        ...
        traffic out this session's real ppp interface on its own.
        (hping3 was tried first here and confirmed unable to transmit
        anything at all over a PPP interface regardless of this routing
        setup -- see bng_flood.py's own module docstring for why; this
        routing infrastructure turned out to still be exactly right for
        genuine kernel-originated traffic, just not for hping3's raw
        L2-framed injection.)"""
    ...
    def start_attack(self, protocol: str, dst_port: int, pps: float, target_ip: str) -> None:
        ...
        if protocol == "ICMP":
            ...
            # L2-framed injection -- already confirmed working over
            # these PPP interfaces throughout this project's debugging,
            # -I bind and all.
            argv = ["ping", "-q", "-i", f"{_ping_interval(pps):.4f}", "-I", self.ppp_iface, target_ip]
        else:
            # TCP_SYN/UDP: bng_flood.py -- real kernel sockets bound to
            # this subscriber's own IP. See its own module docstring for
            # why hping3 can't be used here at all. Relies on the
            # source-based policy route _add_source_route() set up in
            # session_up() to deliver this out the right ppp interface.
            argv = [
